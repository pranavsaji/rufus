...
class RufusClient:
    ...
    # **Removed _allowed_by_robots method**
...
#         """
#         Checks if crawling the given URL is allowed by robots.txt.
...
#             self.logger.debug(f"Robots.txt allows crawling {url}: {allowed}")
#             return allowed
#         except Exception as e:
#             self.logger.error(f"Error accessing robots.txt at {robots_url}: {e}")
#             # Default to disallow if robots.txt cannot be accessed
#             return False
...
# # #             self.browser = await p.chromium.launch(headless=True)
# # #             self.logger.debug(f"Browser launched.")
# # #             await self._crawl(self.base_url, depth=0)
# # #             await self.browser.close()
# # #             self.logger.debug(f"Browser closed.")
# # #         return self.results
...
# # # # class RufusClient:
# # # #     def __init__(self, base_url: str, instructions: str, max_depth: int = 2, max_pages: int = 50, similarity_threshold: float = 0.5):
# # # #         """
# # # #         Initializes the RufusClient.
...
# # # #             content = self._extract_content(html)
# # # #             if content:
# # # #                 is_relevant = compute_similarity(content, self.instructions, self.similarity_threshold)
# # # #                 self.logger.debug(f"Similarity score for URL {url}: {is_relevant}")
# # # #                 if is_relevant:
# # # #                     self.results.append({
# # # #                         "url": url,
...
# # # #     def _is_valid_url(self, url: str) -> bool:
# # # #         """
# # # #         Checks if the URL is within the target domain.

# # # #         :param url: The URL to check.
# # # #         :return: True if valid, False otherwise.
# # # #         """
# # # #         parsed_url = urlparse(url)
# # # #         return parsed_url.netloc.endswith('sfgov.org')
